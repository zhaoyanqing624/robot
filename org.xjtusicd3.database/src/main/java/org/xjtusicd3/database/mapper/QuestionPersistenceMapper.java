...
public interface QuestionPersistenceMapper extends IBaseDao<QuestionPersistence, String>{
	...
	@Select("SELECT FAQQUESTIONID, FAQTITLE, MODIFYTIME, FAQDESCRIPTION ,sum(SCAN+COLLECTION*10) as a FROM TBL_FAQquestion GROUP BY FAQQUESTIONID ORDER BY a DESC LIMIT #{0},5")
