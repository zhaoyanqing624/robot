...
public interface MessagePersistenceMapper extends IBaseDao<MessagePersistence, String>{
	 ...
	 */
	@Select("SELECT * FROM TBL_Message WHERE MESSAGETIME<#{3} AND ((POSTUSERID=#{0} AND GETUSERID=#{1}) OR (POSTUSERID=#{1} AND GETUSERID=#{0})) AND MESSAGESTATE=#{2} ORDER BY MESSAGETIME DESC")
	List<MessagePersistence> getMessageContent11(String postuserId, String getuserId, int state,String date);
	@Select("SELECT * FROM TBL_Message WHERE MESSAGETIME<#{3} AND ((POSTUSERID=#{0} AND GETUSERID=#{1}) OR (POSTUSERID=#{1} AND GETUSERID=#{0}))  AND MESSAGESTATE=#{2} ORDER BY MESSAGETIME DESC LIMIT 5")
